...
| 🌐 | 静态市场 | 生成营销落地页 `index.html`、市场浏览页 `catalog.html`、`catalog.json`、`agents-market.json`，可部署到 GitHub Pages |
...
| 方式 | 命令 | 适合 |
| --- | --- | --- |
| 🍺 Homebrew | `brew install tt-a1i/tap/agents-market` | macOS / Linux 日常使用 |
| ⚡ npx 免安装 | `npx github:tt-a1i/agents-market#preview-0.1.0 <command>` | 一次性试用、CI |
| 🛡️ 校验安装脚本 | `install.sh`（见下方） | 要求 checksum / attestation 验证的环境 |
| 🔌 Claude Code 插件 | `/plugin marketplace add tt-a1i/agents-market` | 在 Claude Code 会话内自助安装 packs |
| 📦 npm | `npm install -g @agents-market/cli` | 即将发布 |
...
Formula 在 [tt-a1i/homebrew-tap](https://github.com/tt-a1i/homebrew-tap)，当前从 commit 固定的 preview 源码归档构建，npm 发布后会切换到 registry tarball。
...
从 GitHub Release 安装，写入前校验 checksum，并用 `npm install -g --ignore-scripts` 跳过 npm 生命周期脚本：

```bash
curl -fsSL https://github.com/tt-a1i/agents-market/releases/download/preview-0.1.0/install.sh | sh
```

要求 GitHub Artifact Attestation 验证（需要 `gh` CLI）：
...
```bash
# 1. 初始化：锁 registry、安装 agent-native 集成、检测项目并给出推荐
agents-market init --target all

# 2. 预览推荐 pack 的审计、策略检查和文件 diff（不写文件）
...
| 图标 | 文件 | 用途 |
| --- | --- | --- |
| 🏠 | `index.html` | 营销落地页（暗色 · IBM Plex · 真实 registry 数据） |
| 🖥️ | `catalog.html` | 面向人类的市场浏览/搜索页面 |
