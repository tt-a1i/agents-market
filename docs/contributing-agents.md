...
- `sourceCommit`, when imported from GitHub or another Git-backed source
- `license`
- `author`, when known
...
`registry:check` builds the CLI and runs the same full review gate. `registry review` runs strict registry lint, checks all first-class target support, audits every pack, previews `apply --json` for every pack under the balanced policy, verifies a catalog built from `./registry`, and can write JSON and Markdown summaries for pull request evidence. The pack table reports imported, checksummed, and source-commit coverage so reviewers can spot weak provenance quickly.
...
- Source repository, source commit when available, source checksum, and license for imported content.
...
- Confirm provenance, source commits, source checksums, and license data are present for third-party content.
