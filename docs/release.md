...
GitHub Actions release, CI, Pages, and registry review workflows run on Node.js 24 and set `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true` so hosted actions use the current JavaScript runtime. The Security workflow intentionally avoids top-level `env`, `defaults`, and write permissions because OpenSSF Scorecard rejects workflow-level configuration when `publish_results` is enabled; write permissions are scoped to the specific scan jobs that need them. Jobs define explicit timeouts and check out source with persisted GitHub credentials disabled. Non-release workflows cancel superseded runs on the same ref; release runs are serialized without canceling an in-progress publish. The Pages workflow also runs `agents-market catalog verify --dir ./site` before uploading the static catalog artifact.
...
The Release workflow serializes runs by release tag or manual version and checks out source with persisted GitHub credentials disabled. Artifact generation, verification, attestation, and GitHub Release uploads run before the protected npm publish job so preview releases can be refreshed without npm credentials. Release jobs declare explicit permissions: the artifact job can write Release assets and attestations, while the npm job has only source read access plus OIDC for `npm publish --provenance`. Only the `publish-npm` job runs in the `npm-release` GitHub Actions environment, and only for `v*` release tags.
